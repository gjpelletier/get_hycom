...
# get_hycom_1hr.ps1 
# 
# Microsoft Powershell script to extract hourly sur/surface diagnostic variables
...
# - - -
# Edit the following as needed to download any subset of these available variables:
...
$resultDirectory = "c:\data\hycom\1hr\"
...
# Use glb = 'GLBy0.08' and expt = '93.0' for dates between 12/4/2018 or 1/1/2019 to present
...
for ($time = $startDate; $time -le $endDate; $time=$time.AddHours(1)){
$error_flag = 1
while ($error_flag -eq 1){

	if($expt -ne '53.X') {
		$url = "https://ncss.hycom.org/thredds/ncss/" + $glb + "/expt_" + $expt + "/sur?var=" + $var_list + "&north=" + $north.ToString() + "&west=" + $west.ToString() + "&east=" + $east.ToString() + "&south=" + $south.ToString() + "&disableLLSubset=on&disableProjSubset=on&horizStride=1&time=" + $time.ToString('yyyy-MM-ddTHH:mm:ssZ') + "&accept=netcdf4"
	}else {
		$url = "http://ncss.hycom.org/thredds/ncss/" + $glb + "/expt_" + $expt + "/data/" + $time.ToString('yyyy') + "/sur?var=" + $var_list + "&north=" + $north.ToString() + "&west=" + $west.ToString() + "&east=" + $east.ToString() + "&south=" + $south.ToString() + "&time=" + $time.ToString('yyyy-MM-ddTHH:mm:ssZ') + "&accept=netcdf4"
	}
